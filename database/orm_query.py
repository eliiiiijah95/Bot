...
from database.models import Customer, Access
...
async def orm_get_category(session: AsyncSession, user_id: int):
    try:
        ...
        print(f'Произошла ошибка при получении информации о клиенте: {e}')
        return None
...
async def orm_get_time_sub(session: AsyncSession, user_id: int):
    try:
        ...
        print(f'Произошла ошибка при получении информации о подписке: {e}')
        return None
...
async def get_type_access(session: AsyncSession, user_id: int):
    try:
        ...
        print(f'Произошла ошибка при получении информации о виде подписки: {e}')
        return None
...
async def save_business_access(session: AsyncSession, user_id: int, access_type: str, subscription_end: int | None):
    try:
        ...
        print(f"Ошибка при сохранении доступа: {e}")
        await session.rollback()
        return False
